...
//    [self addTimer];
   ...
   // 继续向上拖动查看商品详情
    ...
    //创建table
    _table=[[UITableView alloc]initWithFrame:CGRectMake(0,0, self.view.frame.size.width, self.view.frame.size.height+20) style:UITableViewStylePlain];
...
-(void)creatToolBar
{
    ...
    float a = 218.0/750.0;
    CGFloat size=(widthScreen - widthScreen*a*2)/3;
    NSArray * arr;
  //![shopID isEqualToString:@"0"]说明有店铺
    if (![shopID isEqualToString:@"0"]) {
       arr = @[@"客服",@"店铺",@"收藏",@"加入购物车",@"立刻购买"];
    }else{
        arr = @[@"收藏",@"加入购物车",@"立刻购买"];
    }
    UIImage * imagerr;
    ...
    NSString * strrr;
    strrr=[isAttention stringValue];
    GXCustomButton * button=[[GXCustomButton alloc]init];
    if (![shopID isEqualToString:@"0"]) {
        button.frame=CGRectMake(size*2, 0, size, toolHeight);
    }else{
        button.frame=CGRectMake(0, 0, size*3, toolHeight);
    }
    ...
    if ([strrr isEqualToString:@"0"]) {
        ...
        imagerr=[UIImage imageNamed:@"goods_detail_collect"];
    ...
    else if ([strrr isEqualToString:@"1"])
    {
        ...
        button.titleLabel.textAlignment = NSTextAlignmentCenter;
        button.imageView.contentMode = UIViewContentModeCenter;
        imagerr=[UIImage imageNamed:@"goods_detail_collect_press"];
    ...
    if (![shopID isEqualToString:@"0"]) {
        for (int i = 0; i<2; i++) {
            GXCustomButton * button = [[GXCustomButton alloc]init];
            button.frame = CGRectMake(size*i, 0, size, toolHeight);
            ...
            [button setTitleColor:[UIColor colorWithHexString:@"#43464c"] forState:UIControlStateNormal];
            if (i ==0) {
                [button setImage:[UIImage imageNamed:[NSString stringWithFormat:@"goods_detail_service"]] forState:UIControlStateNormal];
                [button setImage:[UIImage imageNamed:[NSString stringWithFormat:@"goods_detail_service"]] forState:UIControlStateHighlighted];
                [button setImage:[UIImage imageNamed:[NSString stringWithFormat:@"商品详情－菜单栏－客服－选中"]] forState:UIControlStateSelected];
            }else if(i==1){
                [button setImage:[UIImage imageNamed:[NSString stringWithFormat:@"goods_detail_shop"]] forState:UIControlStateNormal];
                [button setImage:[UIImage imageNamed:[NSString stringWithFormat:@"goods_detail_shop"]] forState:UIControlStateHighlighted];
                [button setImage:[UIImage imageNamed:[NSString stringWithFormat:@"goods_detail_shop_press"]] forState:UIControlStateSelected];
            }
            button.titleLabel.textAlignment = NSTextAlignmentCenter;
            button.imageView.contentMode = UIViewContentModeCenter;
            button.tag=1502+i;
    ...
    int iarr = (int)arr.count;
    for (int i=iarr-2; i<arr.count; i++)
    {
        UIButton * button=[UIButton buttonWithType:UIButtonTypeCustom];
        ...
        if(i==iarr-2)
        {
            button.frame=CGRectMake(widthScreen - widthScreen*a*2,  0, widthScreen*a, toolHeight);
            button.titleLabel.font = [UIFont systemFontOfSize:15];
            button.backgroundColor=[UIColor colorWithHexString:@"#ff9302"];
        }else if(i == iarr-1){
            //立刻购买
             button.frame=CGRectMake(widthScreen - widthScreen*a,  0, widthScreen*a, toolHeight);
            button.titleLabel.font = [UIFont systemFontOfSize:15];
            button.backgroundColor=[UIColor colorWithHexString:@"#fd5001"];
        }
    ...
    //tabbar线
    UIView *view = [[UIView alloc]initWithFrame:CGRectMake(0, 0, widthScreen, 1)];
    view.backgroundColor = [UIColor colorWithHexString:@"#E5E5E5"];
    [toolView addSubview:view];
    if (![shopID isEqualToString:@"0"]) {
        UIView *view1 = [[UIView alloc]initWithFrame:CGRectMake(size, 0, 1, toolHeight)];
        view1.backgroundColor = [UIColor colorWithHexString:@"#E5E5E5"];
        [toolView addSubview:view1];
        
        UIView *view2 = [[UIView alloc]initWithFrame:CGRectMake(size*2, 0, 1, toolHeight)];
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == 0) {
        ...
        if (shopPrice.length>0) {
            ...
            if (!([shopPrice rangeOfString:@"."].location==NSNotFound)){
                NSMutableAttributedString *priceStr = [[NSMutableAttributedString alloc]initWithString:shopPrice];
                NSUInteger a = priceStr.length - [[priceStr string] rangeOfString:@"."].location;
                NSUInteger b = [[priceStr string] rangeOfString:@"."].location;
                [priceStr addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:19] range:NSMakeRange([[priceStr string] rangeOfString:@"."].location, a)];
                [priceStr addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:24] range:NSMakeRange(0, b)];
                cell.lab2.attributedText= priceStr;
                
            }else{
                cell.lab2.text = shopPrice;
            }
        ...
        cell.monSign.text=@"￥";
        cell.monSign.font = [UIFont systemFontOfSize:19];
        //分享
    ...
    if (btn.tag==1504)
    {
        if (receNs!=NULL) {
            if (buySum == nil) {
                [self.goodVC showHideSlidebar];
                self.goodVC.type = @"1";
                self.goodVC.goodID = self.goodID;
                [UIView animateWithDuration:0.5 animations:^{
                ...
                if (attribute.count == 0) {
                    ...
                    if (_propertyID!=NULL) {
                        ...
                        path1 = [NSString stringWithFormat:@"[%@]",_propertyID];
                    ...
                    // strr=@"0";
                    NSDictionary *dict;
                    dict= @{@"api_token":api_token,@"goods_id":self.goodID,@"key":receNs,@"num":buySum,@"attrvalue_id":path1};
                    ...
                    if (allProperty != nil) {
                        if (notChange.length>0) {
                            ...
                            [RequestModel requestWithDictionary:dict model:@"goods" action:@"addcart" block:^(id result) {
                                UIAlertView * alert=[[UIAlertView alloc]initWithTitle:@"提示" message:result[@"msg"] delegate:nil cancelButtonTitle:@"确定" otherButtonTitles: nil];
                                [alert show];
                            }];
