...
-(void)sendMessage:(id)message
{
    ...
    if ([message[@"code"] isEqualToString:@"0"]) {
        UIAlertView *alert = [[UIAlertView alloc]initWithTitle:nil message:message[@"msg"] delegate:self cancelButtonTitle:nil otherButtonTitles:@"确定", nil];
        [alert show];
...
-(void)creatTab
{
    ...
    monTab.tableFooterView = [[UIView alloc]init];
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.moneyLab.text= [NSString stringWithFormat:@"¥%@",monSource[indexPath.row][@"type_money"]];
    cell.moneyLab.textColor = [UIColor colorWithHexString:@"#ffffff"];
    cell.requireLab.text=[NSString stringWithFormat:@"满%@可用",monSource[indexPath.row][@"min_goods_amount"]];
    cell.requireLab.textColor = [UIColor colorWithHexString:@"#ffffff"];
    cell.stateLab.text=monSource[indexPath.section][@"status"];
    
    NSString * time1=monSource[indexPath.section][@"use_start_date"];
    NSDate * dt = [NSDate dateWithTimeIntervalSince1970:[time1 floatValue]];
    NSDateFormatter * df = [[NSDateFormatter alloc] init];
    [df setDateFormat:@"YYYY.MM.dd"];
    NSString * startTime = [df stringFromDate:dt];
    
    NSString * time2=monSource[indexPath.section][@"use_end_date"];
