...
-(void)deletAction{
    ...
    NSDictionary *dict = @{@"api_token":api_token,@"id_values":deleStr,@"key":app.tempDic[@"data"][@"key"],@"type":@"0"};
...
-(void)myCollectlist{
    ...
    [RequestModel requestWithDictionary:dict model:@"goods" action:@"collectlist" block:^(id result) {
        ...
        for (NSDictionary *dict in dic[@"data"]) {
            ...
            model.goodsComment = dict[@"goods_comment"];
            model.goodRage = dict[@"goods_rage"];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    static NSString *string = @"SearchListTableCell";
    
    SearchListTableCell * cell=[tableView cellForRowAtIndexPath:indexPath];
    if (!cell) {
        cell=[[SearchListTableCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:string];
...
//    [cell.iconImage setImageWithURL:url];
    ...
    NSString *priceStr = model.goodsPrice;
...
//    }
    ...
    //购买人数为空则赋值0
    NSString *string2=[NSString stringWithFormat:@"%@",model.goodRage];
    ...
    NSString *string1=[NSString stringWithFormat:@"%@",model.goodsComment];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (_deletBtn.hidden) {
        ...
        shangpinModel *model;
        model = self.array[indexPath.row];
        NSLog(@"%ld--%@",(long)indexPath.row,model.orderId);
        [_deletArr addObject:model.orderId];
...
-(void)tableView:(UITableView *)tableView didDeselectRowAtIndexPath:(NSIndexPath *)indexPath{
    shangpinModel *model;
    model = self.array[indexPath.row];
    [_deletArr removeObject:model.orderId];
    if (_deletArr.count>0) {
        _deletBtn.backgroundColor = [UIColor colorWithHexString:@"#ff5000"];
        _deletBtn.userInteractionEnabled = YES;
    }else{
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        ...
        AppDelegate *app=appli.delegate;
        NSString *api_token = [RequestModel model:@"goods" action:@"qcollect"];
        NSDictionary *dict = @{@"api_token":api_token,@"id_values":model.orderId,@"key":app.tempDic[@"data"][@"key"],@"type":@"0"};
         __weak typeof(self) weakSelf = self;
        [RequestModel requestWithDictionary:dict model:@"goods" action:@"qcollect" block:^(id result) {
            [weakSelf myCollectlist];
        }];
